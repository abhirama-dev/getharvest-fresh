...
<div x-data="{ balasId: null }">
    <h1 class="text-2xl font-bold text-gray-800 mb-1">Negosiasi Masuk</h1>
    <p class="text-gray-500 mb-6">Tinjau dan respon tawaran harga dari pedagang.</p>

    <?php if (empty($daftarNego)): ?>
        <div class="card p-12 text-center text-gray-400">
            <?= heroicon('chat', 'w-12 h-12 mx-auto mb-3') ?>
            <p>Belum ada tawaran nego masuk.</p>
        </div>
    <?php else: ?>
        <div class="space-y-4">
                ...
                <div class="card p-5">
                    <div class="flex items-center gap-3 mb-3">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-xs text-gray-500">dari <?= esc($n['nama_pedagang']) ?> &middot; harga awal <?= format_rupiah($n['harga_per_kg']) ?>/kg</p>
                        </div>
                    ...
                    <?php if ($n['status_terkini'] === 'Menunggu' && $n['pihak_selanjutnya'] === 'petani'): ?>
                        <div class="flex flex-wrap gap-2">
                            <form action="<?= site_url('petani/nego/respon/' . $n['id_nego_aktif']) ?>" method="post">
                                <?= csrf_field() ?>
                                <input type="hidden" name="aksi" value="terima">
                                <button type="submit" class="btn-primary text-sm py-2">Terima Tawaran</button>
                            </form>
                    ...
                    <?php elseif ($n['status_terkini'] === 'Menunggu'): ?>
                        <p class="text-xs text-gray-400">Menunggu respon pedagang...</p>
                    <?php elseif ($n['status_terkini'] === 'Diterima'): ?>
                        <p class="text-xs text-primary">Disetujui — menunggu pembayaran dari pedagang.</p>
                    <?php endif; ?>
                </div>
            <?php endforeach; ?>
        </div>
    ...
    <div x-show="balasId !== null" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" style="display:none">
        ...
        <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-sm p-6">
            ...
            <form :action="`<?= site_url('petani/nego/respon') ?>/${balasId}`" method="post">
                <?= csrf_field() ?>
